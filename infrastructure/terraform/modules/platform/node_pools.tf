...
resource "google_container_node_pool" "pool" {
  ...
  autoscaling {
    ...
    # Spread nodes over the region's zones rather than filling the cheapest.
    # A matcher owns a shard, so a zone loss should cost a fraction of the
    # shards rather than all of them.
  ...
  upgrade_settings {
    ...
    # matchers whose shards have nowhere else to run.
    strategy        = "SURGE"
    max_surge       = 1
    max_unavailable = 0
  }
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "Pool ${each.key} is spot with no taint. Preemption there takes shards offline, so make the placement deliberate: add a taint and select for it in the chart."
    }
  }
}
