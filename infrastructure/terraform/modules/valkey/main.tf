...
locals {
  ...
  valkey_indices = { for i in range(var.primaries) : format("%03d", i) => i }

  valkey_resources = {
    requests = {
      cpu    = "${var.cpu_millis}m"
      memory = "${var.memory_mib}Mi"
    }
  ...
  valkey_architecture = var.replicas_per_primary > 0 ? "replication" : "standalone"
  ...
  valkey_flags = ["--io-threads", tostring(var.io_threads)]
}
  ...
  values = [yamlencode(merge(
    {
      ...
      replica = merge({
        replicaCount = var.replicas_per_primary
        resources    = local.valkey_resources
        extraFlags   = local.valkey_flags
        persistence  = { enabled = var.persistence }
      }, local.valkey_scheduling)
...
check "valkey_client_can_address_the_fleet" {
  assert {
    condition = var.client_mode == "pooled-hash" || var.primaries == 1
    error_message = join(" ", [
      "valkey_client_mode is 'single' but the fleet has ${var.primaries} primaries,",
